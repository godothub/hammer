...
func flash() -> void:
	...
	### 文件文件
	#var files:PackedStringArray = menu_root.manage_root.game_root.archive_list()
	#
...
func button_clicked(_item: TreeItem, _column: int, _id: int, _mouse_button_index: int) -> void:
	...
	#var game_root:GameRoot = menu_root.manage_root.game_root
	#if _id == 0:
		## 运行
		#game_root.archive_load(file)
		#menu_root.manage_root.game_status_running()
	#elif _id == 1:
		## 删除
